...
async def cmd_login(m: Message):
    ...
    from apple_automation import apple_signin
    cfg = _get_cfg()
    try:
        r = await asyncio.wait_for(
            apple_signin(
                ...
                email=cfg["email"],
                password=cfg["password"],
                q1_text=cfg.get("q1_text", ""),
                q1_answer=cfg.get("q1_answer", ""),
                q2_text=cfg.get("q2_text", ""),
                q2_answer=cfg.get("q2_answer", ""),
                q3_text=cfg.get("q3_text", ""),
                q3_answer=cfg.get("q3_answer", ""),
                tfa_queue=_tfa_queue,
                notify_fn=notify,
        ...
        if r["ok"]:
            ...
            await m.answer(
                ...
                f"Скриншотов: {len(shots)}\n\n"
                "Теперь доступны: /devices /findmy /changepass /mail",
                parse_mode="HTML",
                reply_markup=main_kb(),
            )
            if shots:
                await _send_screenshot(m, shots[-1], "Последний шаг входа")
        else:
